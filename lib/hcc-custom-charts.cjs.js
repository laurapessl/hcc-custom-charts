...
d3Selection.selection.prototype.styles = styles$2;
d3Transition.transition.prototype.styles = styles$2;
...
function styles$2(name, value) {
  ...
  function styleFunction(value) {
    return function () {
      ...
      for (var key in v) {
        addStyle(d3Selection.select(this), key, v[key]);
      }
    };
  }
}

function render$5(node, data, visualOptions, mapping) {
  ...
  const svg = d3Selection.select(node);
  ...
  function createScales() {
    const xScale = d3Scale.scaleLinear().domain(d3Array.extent(xDimension)).range([0, boundWidth]).nice();
    const yScale = d3Scale.scaleLinear().domain(d3Array.extent(yDimension)).range([boundHeight, 0]).nice();
    return {
      xScale,
      yScale
    };
  ...
  function createAxes() {
    const yAxisGenerator = d3Axis.axisLeft().scale(yScale).tickFormat(d3Format.format(".1e"));
    ...
    const xAxisGenerator = d3Axis.axisBottom().scale(xScale).tickFormat(d3Format.format(".1e"));
...
function render$4(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const nestedData = d3Array.groups(data, d => d.series);
  ...
  const seriesSortings = {
    totalDescending: function (a, b) {
      return d3Array.descending(a.totalValue, b.totalValue);
    },
    totalAscending: function (a, b) {
      return d3Array.ascending(a.totalValue, b.totalValue);
    },
    name: function (a, b) {
      return d3Array.ascending(a[0], b[0]);
    }
  };
  // sort series
  ...
  */
  ...
  series.each(function (serie, seriesIndex) {
    ...
    const selection = d3Selection.select(this).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
    ...
    // add series titles
    if (showSeriesLabels) {
      d3Selection.select(this).append('text').attr('y', 4).attr('x', 4).text(d => d[0]).attr('class', 'seriesLabel'); //instead of .styles(styles.seriesLabel)
    }
    ...
    d3Array.extent(data, e => e.y) :
    // compute extent of the single series
    d3Array.extent(serieData, d => d.y);
    if (yOrigin) {
      yDomain[0] = 0;
    }
    ...
    // compute extent of the whole dataset
    d3Array.extent(data, e => e.x) :
    // compute extent of the single series
    d3Array.extent(serieData, d => d.x);
    if (xOrigin) {
      xDomain[0] = 0;
    }
    ...
    // create scales
    // x scale
    const xScale = mapping.x.dataType.type === 'date' ? d3Scale.scaleTime() : d3Scale.scaleLinear();
    xScale.domain(xDomain).rangeRound([0, seriesWidth]).nice();

    // y scale
    const yScale = mapping.y.dataType.type === 'date' ? d3Scale.scaleTime() : d3Scale.scaleLinear();
    yScale.domain(yDomain).rangeRound([seriesHeight, 0]).nice();

    // create axis functions
    // x axis
    const xAxis = g => {
      return g.attr('transform', `translate(0,${seriesHeight})`).call(d3Axis.axisBottom(xScale)).call(g => g.append('text').attr('x', seriesWidth).attr('dy', -5).attr('text-anchor', 'end').text(mapping['x'].value).attr('class', 'axisLabel') //instead of .styles(styles.axisLabel)
      );
    };

    // y axis
    const yAxis = g => {
      return g.call(d3Axis.axisLeft(yScale)).call(g => g.append('text').attr('x', 4).attr('text-anchor', 'start').attr('dominant-baseline', 'hanging').text(mapping['y'].value).attr('class', 'axisLabel') //instead of .styles(styles.axisLabel)
      );
    };
    ...
    if (mapping.connectedBy.value) {
      const line = d3Shape.line().x(d => xScale(d.x)).y(d => yScale(d.y));
      vizLayer.append('path').attr('d', () => line(serieData.sort((a, b) => {
        return d3Array.ascending(a.connectedBy, b.connectedBy);
    ...
    .text((d, i) => {
      if (d && mapping.label.dataType[i].type === 'date') {
        return d3TimeFormat.timeFormat(rawgraphsCore.dateFormats[mapping.label.dataType[i].dateFormat])(d);
      } else {
        return d;
      }
  ...
  // add legend
  if (showLegend) {
    const legendLayer = d3Selection.select(svgNode).append('g').attr('id', 'legend').attr('transform', `translate(${width},${marginTop})`);
...
function render$3(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const svg = d3Selection.select(svgNode).attr('width', showLegend ? width + legendWidth : width).attr('height', height);

  // Add background
  d3Selection.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background');
  const chartGroup = svg.append('g').attr('transform', `translate(${margin.left + chartWidth / 2},${margin.top + chartHeight / 2})`).attr('id', 'viz');

  // Create the pie layout
  const pie = d3Shape.pie().startAngle(rotation).endAngle(Math.PI * 2 + rotation).value(1).sort(null);
  const pieData = pie(data);
  const max_rad = d3Array.max(data, d => d.size) / data.length;
  console.log(pieData);
  console.log(max_rad);
  const arc = d3Shape.arc().innerRadius(0).outerRadius(d => radius * (d.data.size / d3Array.max(data, d => d.size)) + padding * 5);
  chartGroup.selectAll('path').data(pieData).enter().append('path').attr('d', arc).attr('fill', d => colorScale(d.data.color)).attr('stroke', 'white').style('stroke-width', padding).on('mouseover', function (event, d) {
    d3Selection.select(this).attr('opacity', 0.7);
    d3Selection.select(`#label-${d.index}`).style('display', 'block');
  }).on('mouseout', function (event, d) {
    d3Selection.select(this).attr('opacity', 1);
    d3Selection.select(`#label-${d.index}`).style('display', 'none');
  });
  const labelArc = d3Shape.arc().innerRadius(d => radius * (d.data.size / d3Array.max(data, d => d.size)) + 10).outerRadius(d => radius * (d.data.size / d3Array.max(data, d => d.size)) + 10);
  ...
  }).attr('dy', '.35em').attr('text-anchor', 'middle').text(d => d.data.hierarchy.get('month')).style('display', 'none');
  if (title) {
    svg.append('text').attr('x', width / 2).attr('y', margin.top).attr('text-anchor', 'middle').attr('font-size', '16px').attr('font-weight', 'bold').text(title).attr('dy', '1em');
  }
  if (showLabelsOutline) {
  ...
  if (showLegend) {
    const legendLayer = d3Selection.select(svgNode).append('g').attr('id', 'legend').attr('transform', `translate(${width - legendWidth},${marginTop})`);
    const chartLegend = rawgraphsCore.legend().legendWidth(legendWidth);
    if (mapping.color.value) {
      chartLegend.addColor(mapping.color.value, colorScale);
    }
...
function render$2(node, data, visualOptions, mapping, originalData, styles) {
  ...
  function calcProps() {
    ...
    const barsSortings = {
      totalDescending: (a, b) => d3Array.descending(a[1], b[1]),
      totalAscending: (a, b) => d3Array.ascending(a[1], b[1]),
      name: (a, b) => d3Array.ascending(a[0], b[0]),
      original: (a, b) => true
    };
    const barsDomain = d3Array.rollups(data, v => d3Array.sum(v, d => d.size), d => d.y).sort(barsSortings[sortBarsBy]).map(d => d[0]);
    return {
      minTitleHeight,
      titleSize,
      boundWidth,
  ...
  function createScales() {
    const x1Scale = d3Scale.scaleLinear().domain(d3Array.extent(data, x1Accessor)).range([0, boundWidthOneChart / 2]).nice();
    const x1ScaleReverse = d3Scale.scaleLinear().domain(d3Array.extent(data, x1Accessor)).range([boundWidth, (boundWidth + spaceCommonAxis) / 2]).nice();
    const x2Scale = d3Scale.scaleLinear().domain(d3Array.extent(data, x2Accessor)).range([0, boundWidthOneChart / 2]).nice();
    const yScale = d3Scale.scaleBand().domain(barsDomain).range([0, boundHeight]).padding(padding / (boundHeight / barsDomain.length));
    return {
      x1Scale,
      x2Scale,
      x1ScaleReverse,
  ...
  function createAxes() {
    const yAxisGenerator = d3Axis.axisLeft().scale(yScale);
    const yAxis = bounds.append('g').call(yAxisGenerator).attr('text-anchor', 'left');
    ...
    const x1AxisGenerator = d3Axis.axisBottom().scale(x1ScaleReverse);
    ...
    const x2AxisGenerator = d3Axis.axisBottom().scale(x2Scale);
...
function render$1(node, data, visualOptions, mapping, originalData, styles) {
  const {
    background,
    width,
    height,
  ...
  let aggregatedData = d3__namespace.rollups(data, v => d3__namespace.sum(v, d => d.y), d => d.x);
  const sorters = {
    totalDescending: (a, b) => d3__namespace.descending(a[1], b[1]),
    totalAscending: (a, b) => d3__namespace.ascending(a[1], b[1]),
    name: (a, b) => d3__namespace.ascending(a[0], b[0]),
    original: () => 0
  };
  aggregatedData.sort(sorters[sortBarsBy] || sorters.totalDescending);

  // Compute cumulative percentages
  const total = d3__namespace.sum(aggregatedData, d => d[1]);
  ...
  const svg = d3__namespace.select(node);
  ...
  // Scales
  const xScale = d3__namespace.scaleBand().domain(paretoData.map(d => d.category)).range([0, boundWidth]).padding(padding);
  const yScale = d3__namespace.scaleLinear().domain([0, d3__namespace.max(paretoData, d => d.cumulativeSum)]).range([boundHeight, 0]);
  const yScaleRight = d3__namespace.scaleLinear().domain([0, 1]).range([boundHeight, 0]);

  // Bars
  bounds.selectAll('.bar').data(paretoData).join('rect').attr('class', 'bar').attr('x', d => xScale(d.category)).attr('width', xScale.bandwidth()).attr('y', d => yScale(d.value)).attr('height', d => boundHeight - yScale(d.value)).attr('fill', d => barColor);

  // Left y axis (values)
  if (axisLeftLabelVisible) {
    bounds.append('text').attr('transform', `rotate(-90)`).attr('x', -boundHeight / 2).attr('y', -marginLeft + 15).attr('text-anchor', 'middle').styles(styles.axisLabel).text(axisLeftLabel);
  }
  const yAxisLeft = d3__namespace.axisLeft(yScale).ticks(5).tickFormat(d3__namespace.format('~s'));
  bounds.append('g').call(yAxisLeft);

  // Bottom x axis (categories)
  const xAxis = d3__namespace.axisBottom(xScale);
  bounds.append('g').attr('transform', `translate(0,${boundHeight})`).call(xAxis).selectAll('text').attr('text-anchor', labelBottomAlignment).attr('transform', `rotate(${labelBottomRotation})`).attr('dx', '-0.8em').attr('dy', '0.15em');
  if (axisBottomLabelVisible && axisBottomLabel) {
    bounds.append('text').attr('x', boundWidth / 2).attr('y', boundHeight + marginBottom - 10).attr('text-anchor', 'middle').styles(styles.axisLabel).text(axisBottomLabel);
  }

  // Right y axis (cumulative %)
  if (axisRightLabelVisible) {
    bounds.append('text').attr('transform', `rotate(-90)`).attr('x', -boundHeight / 2).attr('y', boundWidth + marginRight - 10).attr('text-anchor', 'middle').styles(styles.axisLabel).text(axisRightLabel);
  }
  const yAxisRight = d3__namespace.axisRight(yScaleRight).ticks(10).tickFormat(d3__namespace.format('.0%'));
  bounds.append('g').attr('transform', `translate(${boundWidth},0)`).call(yAxisRight);

  // Cumulative line
  if (showCumulativeLine) {
    const lineGenerator = d3__namespace.line().x(d => xScale(d.category) + xScale.bandwidth() / 2).y(d => yScaleRight(d.cumulative));
    bounds.append('path').datum(paretoData).attr('fill', 'none').attr('stroke', cumulativeLineColor).attr('stroke-width', 2).attr('d', lineGenerator);
    bounds.selectAll('.cumulative-point').data(paretoData).join('circle').attr('class', 'cumulative-point').attr('cx', d => xScale(d.category) + xScale.bandwidth() / 2).attr('cy', d => yScaleRight(d.cumulative)).attr('r', 4).attr('fill', cumulativeLineColor);
  }
}
...
const visualOptions$1 = {
  ...
  labelBottomAlignment: {
    // renamed for x-axis label alignment
    type: 'text',
...
function render(svgNode, data, visualOptions, mapping, originalData, styles) {
  d3__namespace.select(svgNode).selectAll('*').remove();
  const {
    width,
    chartHeight,
    chartSpacing,
  ...
  const totalHeight = marginTop + marginBottom + data.length * chartHeight + (data.length - 1) * chartSpacing;
  const innerWidth = width - marginLeft - marginRight;
  const svg = d3__namespace.select(svgNode).attr('width', width).attr('height', totalHeight);
  ...
  let globalMax = 0;
  if (globalScale) {
    globalMax = d3__namespace.max(processedData, d => Math.max(d.measure, d.target, ...d.ranges));
  }
  processedData.forEach((d, i) => {
    const yOffset = i * (chartHeight + chartSpacing);
    const maxValue = globalScale ? globalMax : Math.max(d.measure, d.target, ...d.ranges);
    const xScale = d3__namespace.scaleLinear().domain([0, maxValue]).range([0, innerWidth]);
    const chartGroup = g.append('g').attr('transform', `translate(0,${yOffset})`).attr('class', `bullet-chart-${i}`);
    d.ranges.forEach((range, idx) => {
      const colorIndex = Math.min(idx, rangeColors.length - 1);
      chartGroup.append('rect').attr('class', `range range-${idx}`).attr('x', 0).attr('y', 0).attr('width', xScale(range)).attr('height', chartHeight).attr('fill', rangeColors[colorIndex]).attr('opacity', 0.7).attr('stroke', '#fff').attr('stroke-width', 0.5);
    });
    chartGroup.append('rect').attr('class', 'measure').attr('x', 0).attr('y', chartHeight * 0.25).attr('width', xScale(d.measure)).attr('height', chartHeight * 0.5).attr('fill', measureColor).attr('stroke', d3__namespace.rgb(measureColor).darker(0.5)).attr('stroke-width', 1);
    if (d.target > 0) {
      chartGroup.append('line').attr('class', 'target-line').attr('x1', xScale(d.target)).attr('x2', xScale(d.target)).attr('y1', 0).attr('y2', chartHeight).attr('stroke', targetColor).attr('stroke-width', 3).attr('opacity', 0.8);
    }
    chartGroup.append('text').attr('class', 'chart-label').attr('x', -10).attr('y', chartHeight / 2).attr('text-anchor', 'end').attr('dominant-baseline', 'middle').text(d.label).style('font-size', '12px').style('font-weight', 'bold').style('fill', '#333');
    if (showValues) {
      chartGroup.append('text').attr('class', 'measure-value').attr('x', xScale(d.measure) + 5).attr('y', chartHeight / 2).attr('dominant-baseline', 'middle').text(d3__namespace.format(',.0f')(d.measure)).style('font-size', '10px').style('font-weight', 'bold').style('fill', measureColor);
    }
    if (showPerformanceIndicators && d.target > 0) {
      ...
      chartGroup.append('text').attr('class', 'performance-indicator').attr('x', innerWidth + 50).attr('y', chartHeight / 2).attr('dominant-baseline', 'middle').text(performanceText).style('font-size', '10px').style('font-weight', 'bold').style('fill', performanceColor);
    }
    if (showTicks && (i === processedData.length - 1 || globalScale)) {
      ...
      tickValues.forEach(tick => {
        tickGroup.append('line').attr('x1', xScale(tick)).attr('x2', xScale(tick)).attr('y1', chartHeight).attr('y2', chartHeight + 5).attr('stroke', '#666').attr('stroke-width', 0.5);
        tickGroup.append('text').attr('x', xScale(tick)).attr('y', chartHeight + 18).attr('text-anchor', 'middle').text(d3__namespace.format(',.0f')(tick)).style('font-size', '10px').style('fill', '#666');
      });
    }
  });
  if (processedData.length > 1) {
    svg.append('text').attr('x', width / 2).attr('y', 15).attr('text-anchor', 'middle').text(`Performance Dashboard (${processedData.length} metrics)`).style('font-size', '14px').style('font-weight', 'bold').style('fill', '#333');
  }
}
...
exports.barchartpaired = barchartpaired;
exports.bulletchart = bulletchart;
exports.connectedscatterplot = connectedscatterplot;
exports.paretochart = paretochart;
exports.polarareadiagram = polarareadiagram;
exports.similaritymap = similaritymap;
