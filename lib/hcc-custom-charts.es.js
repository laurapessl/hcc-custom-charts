...
function render$4(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  */
  ...
  series.each(function (serie, seriesIndex) {
    ...
    const selection = select(this).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
    ...
    // add series titles
    if (showSeriesLabels) {
      select(this).append('text').attr('y', 4).attr('x', 4).text(d => d[0]).attr('class', 'seriesLabel'); //instead of .styles(styles.seriesLabel)
    }
...
function render$3(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const svg = select(svgNode).attr('width', showLegend ? width + legendWidth : width).attr('height', height);
  ...
  const pie$1 = pie().startAngle(rotation).endAngle(Math.PI * 2 + rotation).value(1).sort(null);
  const pieData = pie$1(data);
  ...
  const labelArc = arc().innerRadius(d => radius * (d.data.size / max(data, d => d.size)) + 10).outerRadius(d => radius * (d.data.size / max(data, d => d.size)) + 10);
  ...
  if (showLegend) {
    const legendLayer = select(svgNode).append('g').attr('id', 'legend').attr('transform', `translate(${width - legendWidth},${marginTop})`);
    const chartLegend = legend().legendWidth(legendWidth);
    if (mapping.color.value) {
      chartLegend.addColor(mapping.color.value, colorScale);
    }
...
function render$1(node, data, visualOptions, mapping, originalData, styles) {
  ...
  let aggregatedData = d3.rollups(data, v => d3.sum(v, d => d.y), d => d.x);
  const sorters = {
  ...
  // Scales
  const xScale = d3.scaleBand().domain(paretoData.map(d => d.category)).range([0, boundWidth]).padding(padding);
  const yScale = d3.scaleLinear().domain([0, d3.max(paretoData, d => d.cumulativeSum)]).range([boundHeight, 0]);
  const yScaleRight = d3.scaleLinear().domain([0, 1]).range([boundHeight, 0]);
  ...
  const yAxisRight = d3.axisRight(yScaleRight).ticks(10).tickFormat(d3.format('.0%'));
  bounds.append('g').attr('transform', `translate(${boundWidth},0)`).call(yAxisRight);

  // Cumulative line
  if (showCumulativeLine) {
    const lineGenerator = d3.line().x(d => xScale(d.category) + xScale.bandwidth() / 2).y(d => yScaleRight(d.cumulative));
...
function render(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const svg = d3.select(svgNode).attr('width', width).attr('height', totalHeight);
  ...
  let globalMax = 0;
  if (globalScale) {
    globalMax = d3.max(processedData, d => Math.max(d.measure, d.target, ...d.ranges));
  }
  processedData.forEach((d, i) => {
    ...
    chartGroup.append('rect').attr('class', 'measure').attr('x', 0).attr('y', chartHeight * 0.25).attr('width', xScale(d.measure)).attr('height', chartHeight * 0.5).attr('fill', measureColor).attr('stroke', d3.rgb(measureColor).darker(0.5)).attr('stroke-width', 1);
    if (d.target > 0) {
    ...
    if (showValues) {
      chartGroup.append('text').attr('class', 'measure-value').attr('x', xScale(d.measure) + 5).attr('y', chartHeight / 2).attr('dominant-baseline', 'middle').text(d3.format(',.0f')(d.measure)).style('font-size', '10px').style('font-weight', 'bold').style('fill', measureColor);
    }
    ...
    if (showTicks && (i === processedData.length - 1 || globalScale)) {
      ...
      tickValues.forEach(tick => {
        ...
        tickGroup.append('text').attr('x', xScale(tick)).attr('y', chartHeight + 18).attr('text-anchor', 'middle').text(d3.format(',.0f')(tick)).style('font-size', '10px').style('fill', '#666');
      });
    }
  });
...
export { barchartpaired, bulletchart, connectedscatterplot, paretochart, polarareadiagram, similaritymap };
