...
d3.transition.prototype.styles = styles$1;
...
function render$2(node, data, visualOptions, mapping) {
  ...
  function createScales() {
    const xScale = d3__namespace.scaleLinear().domain(d3__namespace.extent(xDimension)).range([0, boundWidth]).nice();
    const yScale = d3__namespace.scaleLinear().domain(d3__namespace.extent(yDimension)).range([boundHeight, 0]).nice();
    return {
      xScale,
      yScale
    };
  ...
  function createAxes() {
    const yAxisGenerator = d3__namespace.axisLeft().scale(yScale).tickFormat(d3__namespace.format(".1e"));
    ...
    const xAxisGenerator = d3__namespace.axisBottom().scale(xScale).tickFormat(d3__namespace.format(".1e"));
...
function render$1(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const nestedData = d3__namespace.groups(data, d => d.series);
  ...
  const seriesSortings = {
    totalDescending: function (a, b) {
      return d3__namespace.descending(a.totalValue, b.totalValue);
    },
    totalAscending: function (a, b) {
      return d3__namespace.ascending(a.totalValue, b.totalValue);
    },
    name: function (a, b) {
      return d3__namespace.ascending(a[0], b[0]);
    }
  };
  // sort series
  ...
  const svg = d3__namespace.select(svgNode);
  ...
  */
  ...
  series.each(function (serie, seriesIndex) {
    ...
    const selection = d3__namespace.select(this).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
    ...
    // add series titles
    if (showSeriesLabels) {
      d3__namespace.select(this).append('text').attr('y', 4).attr('x', 4).text(d => d[0]).styles(styles.seriesLabel);
    }
    ...
    d3__namespace.extent(data, e => e.y) :
    // compute extent of the single series
    d3__namespace.extent(serieData, d => d.y);
    if (yOrigin) {
      yDomain[0] = 0;
    }
    ...
    // compute extent of the whole dataset
    d3__namespace.extent(data, e => e.x) :
    // compute extent of the single series
    d3__namespace.extent(serieData, d => d.x);
    if (xOrigin) {
      xDomain[0] = 0;
    }
    ...
    // create scales
    // x scale
    const xScale = mapping.x.dataType.type === 'date' ? d3__namespace.scaleTime() : d3__namespace.scaleLinear();
    xScale.domain(xDomain).rangeRound([0, seriesWidth]).nice();

    // y scale
    const yScale = mapping.y.dataType.type === 'date' ? d3__namespace.scaleTime() : d3__namespace.scaleLinear();
    yScale.domain(yDomain).rangeRound([seriesHeight, 0]).nice();

    // create axis functions
    // x axis
    const xAxis = g => {
      return g.attr('transform', `translate(0,${seriesHeight})`).call(d3__namespace.axisBottom(xScale)).call(g => g.append('text').attr('x', seriesWidth).attr('dy', -5).attr('text-anchor', 'end').text(mapping['x'].value).styles(styles.axisLabel));
    };

    // y axis
    const yAxis = g => {
      return g.call(d3__namespace.axisLeft(yScale)).call(g => g.append('text').attr('x', 4).attr('text-anchor', 'start').attr('dominant-baseline', 'hanging').text(mapping['y'].value).styles(styles.axisLabel));
    };
    ...
    if (mapping.connectedBy.value) {
      const line = d3__namespace.line().x(d => xScale(d.x)).y(d => yScale(d.y));
      vizLayer.append('path').attr('d', () => line(serieData.sort((a, b) => {
        return d3__namespace.ascending(a.connectedBy, b.connectedBy);
    ...
    .text((d, i) => {
      if (d && mapping.label.dataType[i].type === 'date') {
        return d3__namespace.timeFormat(rawgraphsCore.dateFormats[mapping.label.dataType[i].dateFormat])(d);
      } else {
        return d;
      }
  ...
  // add legend
  if (showLegend) {
    const legendLayer = d3__namespace.select(svgNode).append('g').attr('id', 'legend').attr('transform', `translate(${width},${marginTop})`);
...
function render(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const svg = d3__namespace.select(svgNode).attr('width', showLegend ? width + legendWidth : width).attr('height', height);

  // Add background
  d3__namespace.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background');
  const chartGroup = svg.append('g').attr('transform', `translate(${margin.left + chartWidth / 2},${margin.top + chartHeight / 2})`).attr('id', 'viz');

  // Create the pie layout
  const pie = d3__namespace.pie().startAngle(rotation).endAngle(Math.PI * 2 + rotation).value(1).sort(null);

  // Map data for pie chart
  const pieData = pie(data);
  const max_rad = d3__namespace.max(data, d => d.size) / data.length;
  console.log(pieData);
  console.log(max_rad);

  // Create the arc
  const arc = d3__namespace.arc().innerRadius(0).outerRadius(d => radius * (d.data.size / d3__namespace.max(data, d => d.size)) + padding * 5);
  console.log(arc);
  ...
  chartGroup.selectAll('path').data(pieData).enter().append('path').attr('d', arc).attr('fill', d => colorScale(d.data.color)).attr('stroke', 'white').style('stroke-width', padding).on('mouseover', function (event, d) {
    d3__namespace.select(this).attr('opacity', 0.7);
    d3__namespace.select(`#label-${d.index}`).style('display', 'block');
  }).on('mouseout', function (event, d) {
    d3__namespace.select(this).attr('opacity', 1);
    d3__namespace.select(`#label-${d.index}`).style('display', 'none');
  });

  // Create the arc for label placement
  const labelArc = d3__namespace.arc().innerRadius(d => radius * (d.data.size / d3__namespace.max(data, d => d.size)) + 10) // Adjust 10 to control distance from the arc
  .outerRadius(d => radius * (d.data.size / d3__namespace.max(data, d => d.size)) + 10);
  ...
  if (showLegend) {
    const legendLayer = d3__namespace.select(svgNode).append('g').attr('id', 'legend').attr('transform', `translate(${width - legendWidth},${marginTop})`);
    const chartLegend = rawgraphsCore.legend().legendWidth(legendWidth);
    if (mapping.color.value) {
      chartLegend.addColor(mapping.color.value, colorScale);
    }
