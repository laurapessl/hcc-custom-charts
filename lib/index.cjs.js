...
d3.selection.prototype.styles = styles$2;
d3.transition.prototype.styles = styles$2;
...
function render$1(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  d3Shape.pie().startAngle(rotation).endAngle(Math.PI * 2 + rotation).value(1).sort(null);

  // Map data for pie chart
  const pieData = d3Shape.pie(data);
  ...
  const labelArc = d3Shape.arc().innerRadius(d => radius * (d.data.size / d3Array.max(data, d => d.size)) + 10) // Adjust 10 to control distance from the arc
  .outerRadius(d => radius * (d.data.size / d3Array.max(data, d => d.size)) + 10);
