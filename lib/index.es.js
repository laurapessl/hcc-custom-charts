...
import { extent, groups, descending, ascending, rollups, max } from 'd3-array';
  ...
  */
  ...
  series.each(function (serie, seriesIndex) {
    ...
    if (mapping.connectedBy.value) {
      const line = line().x(d => xScale(d.x)).y(d => yScale(d.y));
...
function render(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const pie = pie().startAngle(rotation).endAngle(Math.PI * 2 + rotation).value(1).sort(null);
  ...
  // Create the arc
  const arc = arc().innerRadius(0).outerRadius(d => radius * (d.data.size / max(data, d => d.size)) + padding * 5);
  console.log(arc);
