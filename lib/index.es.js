...
transition.prototype.styles = styles$1;
...
function render$2(node, data, visualOptions, mapping) {
  ...
  function createScales() {
    const xScale = d3.scaleLinear().domain(d3.extent(xDimension)).range([0, boundWidth]).nice();
    const yScale = d3.scaleLinear().domain(d3.extent(yDimension)).range([boundHeight, 0]).nice();
    return {
      xScale,
      yScale
    };
  ...
  function createAxes() {
    const yAxisGenerator = d3.axisLeft().scale(yScale).tickFormat(d3.format(".1e"));
    ...
    const xAxisGenerator = d3.axisBottom().scale(xScale).tickFormat(d3.format(".1e"));
...
function render$1(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  */
  ...
  series.each(function (serie, seriesIndex) {
    ...
    const selection = d3.select(this).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
...
function render(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const svg = d3.select(svgNode).attr('width', showLegend ? width + legendWidth : width).attr('height', height);
  ...
  const pie = d3.pie().startAngle(rotation).endAngle(Math.PI * 2 + rotation).value(1).sort(null);
  ...
  // Create the arc
  const arc = d3.arc().innerRadius(0).outerRadius(d => radius * (d.data.size / d3.max(data, d => d.size)) + padding * 5);
  console.log(arc);
  ...
  const labelArc = d3.arc().innerRadius(d => radius * (d.data.size / d3.max(data, d => d.size)) + 10) // Adjust 10 to control distance from the arc
  ...
  if (showLegend) {
    const legendLayer = d3.select(svgNode).append('g').attr('id', 'legend').attr('transform', `translate(${width - legendWidth},${marginTop})`);
    const chartLegend = legend().legendWidth(legendWidth);
    if (mapping.color.value) {
      chartLegend.addColor(mapping.color.value, colorScale);
    }
...
export { connectedscatterplot, polarareadiagram, similaritymap };
