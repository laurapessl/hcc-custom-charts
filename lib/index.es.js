...
function render(node, data, visualOptions, mapping, originalData, styles) {
  ...
  const yScale = d3.scaleLinear().domain([0, d3.max(paretoData, d => d.value)]).range([boundHeight, 0]).nice();
  ...
  const yAxisRight = d3.axisRight(yScaleRight).ticks(5).tickFormat(d3.format('.0%'));
...
export { barchartpaired, connectedscatterplot, paretochart, polarareadiagram, similaritymap };
